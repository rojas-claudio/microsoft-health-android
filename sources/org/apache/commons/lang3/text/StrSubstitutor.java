        ...
        while (pos < bufEnd) {
            ...
            if (startMatchLen == 0) {
                ...
                while (true) {
                    if (pos >= bufEnd) {
                        break;
                    } else if (isEnableSubstitutionInVariables() && (endMatchLen = prefixMatcher.isMatch(chars, pos, offset, bufEnd)) != 0) {
                        nestedVarCount++;
                        pos += endMatchLen;
                    } else {
                        int endMatchLen2 = suffixMatcher.isMatch(chars, pos, offset, bufEnd);
                        if (endMatchLen2 == 0) {
                            pos++;
                            ...
                            String varValue = resolveVariable(varName, buf, startPos, pos);
                            if (varValue != null) {
                                int varLen = varValue.length();
                                buf.replace(startPos, pos, varValue);
                                altered = true;
                                int change = (substitute(buf, startPos, varLen, priorVariables) + varLen) - (pos - startPos);
                                pos += change;
                                bufEnd += change;
                                lengthChange += change;
                                chars = buf.buffer;
