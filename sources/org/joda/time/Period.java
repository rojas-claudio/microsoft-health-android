...
public final class Period extends BasePeriod implements ReadablePeriod, Serializable {
    ...
    public Weeks toStandardWeeks() {
        ...
        return Weeks.weeks(FieldUtils.safeToInt((((((getMillis() + (getSeconds() * 1000)) + (getMinutes() * DateUtils.MILLIS_PER_MINUTE)) + (getHours() * 3600000)) + (getDays() * 86400000)) / 604800000) + getWeeks()));
    }

    public Days toStandardDays() {
        checkYearsAndMonths("Days");
        return Days.days(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd((((getMillis() + (getSeconds() * 1000)) + (getMinutes() * DateUtils.MILLIS_PER_MINUTE)) + (getHours() * 3600000)) / 86400000, getDays()), getWeeks() * 7)));
    }

    public Hours toStandardHours() {
        checkYearsAndMonths("Hours");
        return Hours.hours(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(((getMillis() + (getSeconds() * 1000)) + (getMinutes() * DateUtils.MILLIS_PER_MINUTE)) / 3600000, getHours()), getDays() * 24), getWeeks() * 168)));
    }

    public Minutes toStandardMinutes() {
        checkYearsAndMonths("Minutes");
        return Minutes.minutes(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd((getMillis() + (getSeconds() * 1000)) / DateUtils.MILLIS_PER_MINUTE, getMinutes()), getHours() * 60), getDays() * 1440), getWeeks() * 10080)));
    }
    ...
    public Duration toStandardDuration() {
        ...
        return new Duration(getMillis() + (getSeconds() * 1000) + (getMinutes() * DateUtils.MILLIS_PER_MINUTE) + (getHours() * 3600000) + (getDays() * 86400000) + (getWeeks() * 604800000));
    }
    ...
    public Period normalizedStandard(PeriodType periodType) {
        PeriodType periodType2 = DateTimeUtils.getPeriodType(periodType);
        Period period = new Period(getMillis() + (getSeconds() * 1000) + (getMinutes() * DateUtils.MILLIS_PER_MINUTE) + (getHours() * 3600000) + (getDays() * 86400000) + (getWeeks() * 604800000), periodType2, ISOChronology.getInstanceUTC());
        int years = getYears();
