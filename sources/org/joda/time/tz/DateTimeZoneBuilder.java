...
public class DateTimeZoneBuilder {
    ...
    static void writeMillis(DataOutput dataOutput, long j) throws IOException {
        if (j % Constants.BACKGROUND_SYNC_INTERVAL_MS == 0) {
            long j2 = j / Constants.BACKGROUND_SYNC_INTERVAL_MS;
            if (((j2 << 58) >> 58) == j2) {
    ...
    static long readMillis(DataInput dataInput) throws IOException {
        ...
        switch (readUnsignedByte >> 6) {
            case 1:
                return (((readUnsignedByte << 26) >> 2) | (dataInput.readUnsignedByte() << 16) | (dataInput.readUnsignedByte() << 8) | dataInput.readUnsignedByte()) * DateUtils.MILLIS_PER_MINUTE;
            case 2:
            ...
            default:
                return ((readUnsignedByte << 26) >> 26) * Constants.BACKGROUND_SYNC_INTERVAL_MS;
        }
    }
    ...
    private boolean addTransition(ArrayList<Transition> arrayList, Transition transition) {
        ...
        if (transition.isTransitionFrom(transition2)) {
            int wallOffset = size >= 2 ? arrayList.get(size - 2).getWallOffset() : 0;
            if (transition.getMillis() + transition2.getWallOffset() != wallOffset + transition2.getMillis()) {
                arrayList.add(transition);
                return true;
            }
    ...
    public static final class Rule {
        ...
        public long next(long j, int i, int i2) {
            ...
            long next = this.iRecurrence.next(i3 < this.iFromYear ? (instanceUTC.year().set(0L, this.iFromYear) - i4) - 1 : j, i, i2);
    ...
    public static final class PrecalculatedZone extends DateTimeZone {
        ...
        static PrecalculatedZone create(String str, boolean z, ArrayList<Transition> arrayList, DSTZone dSTZone) {
            ...
            while (true) {
                ...
                long j3 = iArr2[i4];
                long j4 = iArr2[i4 + 1];
                Period period = new Period(jArr[i4], jArr[i4 + 1], PeriodType.yearMonthDay(), instanceUTC);
                if (j != j2 && j3 == j4 && str2.equals(str3) && period.getYears() == 0 && period.getMonths() > 4 && period.getMonths() < 8 && str2.equals(strArr[2]) && str2.equals(strArr[4])) {
                    if (ZoneInfoCompiler.verbose()) {
                        ...
                        System.out.println("     - " + new DateTime(jArr[i4], instanceUTC) + " - " + new DateTime(jArr[i4 + 1], instanceUTC));
                    }
                    if (j > j2) {
