...
public class StatementExecutor<T, ID> implements GenericRowMapper<String[]> {
    ...
    public <CT> CT callBatchTasks(DatabaseConnection connection, boolean saved, Callable<CT> callable) throws SQLException {
        if (this.databaseType.isBatchUseTransaction()) {
            return (CT) TransactionManager.callInTransaction(connection, saved, this.databaseType, callable);
        }
        boolean autoCommitAtStart = false;
        try {
            if (connection.isAutoCommitSupported() && (autoCommitAtStart = connection.isAutoCommit())) {
    ...
    public static class UserRawRowMapper<UO> implements GenericRowMapper<UO> {
        ...
        public UO mapRow(DatabaseResults results) throws SQLException {
            ...
            return this.mapper.mapRow(getColumnNames(results), stringResults);
        }
